...
import Compiler.Backend.X86.Liveness qualified as L (ControlFlow (..), ControlFlowNode (..), LiveVariables (..), LiveVariablesMap, newControlFlowGraph, solveDataFlowEquation)
...
newInterferenceGraph :: Ord var => Set.Set var -> L.LiveVariablesMap var val -> InterferenceGraph var
...
buildInterfereceGraph vars flows = newInterferenceGraph vars . L.solveDataFlowEquation $ L.newControlFlowGraph flows
