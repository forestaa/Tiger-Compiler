module Frontend.Language.Tiger.IntegrationSpec (spec) where
...
import Frontend.Language.Tiger.Semant.MarkEscape
...
integrationSpec = describe "integration test for translate" $ do
  ...
  it "valid test cases" $ do
    let testcases = (++) <$> (("test/Frontend/Language/Tiger/samples/test" ++) <$> validTestCases) <*> [".tig"]
    res <- runExceptT (traverse translateTest testcases)
    res `shouldSatisfy` isRight

  it "merge.tig" $ do
    let merge = "test/Frontend/Language/Tiger/samples/merge.tig"
    res <- runExceptT (translateTest merge)
    res `shouldSatisfy` isRight

  it "queens.tig" $ do
    let merge = "test/Frontend/Language/Tiger/samples/queens.tig"
    res <- runExceptT (translateTest merge)
    res `shouldSatisfy` isRight
  where
    testcase s = "test/Frontend/Language/Tiger/samples/" ++ s
