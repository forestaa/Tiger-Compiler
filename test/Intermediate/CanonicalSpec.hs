module Intermediate.CanonicalSpec (spec) where

import Data.Extensible.Effect (leaveEff)
import Intermediate.Canonical (Block (..), basicBlocks, linearize, traceSchedule)
import Intermediate.IR
import Intermediate.Unique qualified as U (Label (..), Temp (..), Unique (..), evalUniqueEff)
...
linearizeSpec = describe "linearize spec" $ do
  it "0 -> []" $ do
    let stm = Exp (Const 0)
        result = leaveEff . U.evalUniqueEff @"temp" $ linearize stm
    result `shouldBe` []
...
basicBlocksSpec = describe "basicBlocks spec" $ do
  it "empty" $ do
    let stms = []
        result = leaveEff . U.evalUniqueEff @"label" $ basicBlocks stms
    fst result `shouldBe` []
